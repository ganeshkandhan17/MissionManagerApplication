...
app.get('/', (req, res) => {
    res.send("<script>window.location.href='http://127.0.0.1:3000/login'</script>")
});
        ...
        if (length1 == 0) {
            ...
            registered.save()
                .then(() => {
                    ...
                    res.send("<script>alert('Account Created Successful');window.location.href = 'http://127.0.0.1:3000/login';</script>")
                })
                .catch((err) => {
                    console.log("error occurs in data adding to database")
                });
        } else {
            res.send("<script>alert('Account already exists with this mailid');window.location.href='http://127.0.0.1:3000/signup'</script>");
        }
    });
})
app.post('/login', (req, res) => {
    ...
    register.find({ emailid: getmailid, password: getpassword })
        .then((result) => {
            ...
            let loginedemailid = "";
            if (length1 == 0) {
                res.send("<script>alert('Invalid Email Id or Password');window.location.href='http://127.0.0.1:3000/login'</script>")
            }
            else {
        ...
        .catch((err) => {
            ...
            res.send('<script>alert("Error Occurs in Login");window.location.href="http://127.0.0.1:3000/login"</script>')
        })
})
...
app.post('/forgetpass', (req, res) => {
    ...
    res.send("<script>alert('Instructions Send to the Registered EmailId');window.location.href = 'http://127.0.0.1:3000/login';</script>")
})
        ...
        .then((data) => {
            ...
            if (oldpassword == currentpassword) {
                register.updateOne({ emailid: email }, { password: newpassword })
                    .then((data) => {
                        if (data.modifiedCount == 1) {
                            res.send(`<script>alert('Password Changed Successful');window.location.href="http://127.0.0.1:3000/login"</script>`)
                        }

                    })
            }
            else{
                res.send(`<script>alert('Current Password was Incorrect');window.location.href="http://127.0.0.1:3000/changepassword"</script>`)
            }
        })
})
app.get('*', (req, res) => {
